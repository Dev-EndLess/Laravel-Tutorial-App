    ...
    <div class="mx-4">
        <x-card class="p-10 max-w-lg mx-auto mt-24">
            <header class="text-center">
                ...
                <p class="mb-4">Change or update informations about {{$band->name}}</p>
            </header>

            <form method="POST" action="/bands/{{ $band->id }}" enctype="multipart/form-data">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" class="border border-gray-200 rounded p-2 w-full" name="ticket"
                        value="{{ $band->ticket }}" placeholder="Example: 20€" />
                    @error('ticket')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    ...
                    <input type="text" class="border border-gray-200 rounded p-2 w-full" name="location"
                        value="{{ $band->location }}" placeholder="Example: Bologna, Estragon Club" />
                    @error('location')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    ...
                    <textarea class="border border-gray-200 rounded p-2 w-full" name="description" value="{{ $band->description }}"
